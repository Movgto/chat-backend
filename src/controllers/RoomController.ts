...
class RoomController {
    ...
    static findPublicRoom(cb: Function) {
    ...
    static createRoom(socket: Socket, args: CreateRoomArgs, cb: Function) {
    ...
    static joinRoom(socket: Socket, args: JoinRoomArgs, cb: Function) {
        const roomIdx = this.rooms.findIndex(r => r.id === args.roomId)

        if (roomIdx === -1) return cb({success: false, message: "Couldn't find the room"})
    ...
    static newMessage(args: SendMessageArgs, cb: Function) {
        const room = this.rooms.find(r => r.id === args.roomId)

        if (!room) return cb({success: false, message: 'The room doesn\'t exist'})
